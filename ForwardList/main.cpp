...
{
	T Data;        //Значение элемента. Грубо говоря, что хранится в элементе.
	Element<T>* pNext; //Указатель на следующий элемент. А с помощью этого указателя, элементы связываются в список.
	static int count;
public:
...
//////// Class definition end (Конец бъявление класса)	//////////
/////////////////////////////////////////////////////////////////
...
template<typename T>int Element<T>::count = 0;  //Инициализация стат.переменной так же является определением класса.
...
template<typename T>ConstIterator<T>::~ConstIterator()//память как у рыбы щас ау НА ЯЗЫКЕ КРУТИТСЯ в конст ну дап
{
	cout << "ItDestructor:\t" << this << endl;
}
...
template<typename T>ForwardList<T>::ForwardList(const ForwardList<T>&& other) :ForwardList()
{
	...
	other.Head = nullptr;
	other.size = 0;*/
	*this = std::move(other);  //Метод 'std::move()' вызывает MoveAssigment.
